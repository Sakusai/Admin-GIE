...
function install()
{
  ...
  $wpdb->query(
    "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}annuaire_lieu (
...
function BackOfficeLieux()
{
  ...
  echo '  <label class="screen-reader-text" for="lieu-search-input">Rechercher un lieu :</label>';
  echo '  <input type="search" id="lieu-search-input" name="search" placeholder="Rechercher">';
  echo '  <input type="submit" name="search-submit" id="search-submit" class="button" value="Rechercher">';
  echo '</p>';
  echo '</form>';
...
function BackOfficeLieuMod()
{
  ...
  if (!($enreg === FALSE)) {
    ...
    foreach ($rowcatid as $valcatid) {
      $selected = "";
      if ($enreg->annuaire_cat_id === $valcatid->annuaire_cat_id) {
        $selected = " selected";
      }
